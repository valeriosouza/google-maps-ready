...
function gmpSaveEditedMap() {
	...
	,	onSuccess: function(res) {
*/
			/*if(markerArr && res.data.updateMarkers) {
				console.log('lalala');
				for(var i in res.data.updateMarkers) {
					if(!res.data.updateMarkers[i].tmp_id) continue;
					var tmpId = res.data.updateMarkers[i].tmp_id;
					console.log(tmpId);
					if(markerArr[tmpId]) {
						markerArr[ res.data.updateMarkers[i].id ] = markerArr[tmpId];
						delete markerArr[tmpId];
					}
				}
			}*/
...
jQuery(document).ready(function() {
	...
	jQuery('#gmpAddMarkerToEditMap').submit(function(){
		...
		if(typeof(edit_marker_id) != 'undefined' && edit_marker_id != '') {
			...
			res = afterMarkerFormSubmit('gmpAddMarkerToEditMap');
		}
		if(res) {
			var currentMapId = parseInt(jQuery('#gmpSaveEditedMap').attr('current_map_id'));
			if(currentMapId) {
				gmpSaveEditedMap(currentMapId);
			}
		}*/
	});
